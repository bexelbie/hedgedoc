...
function colorFromUserId (userId) {
  if (!userId) return normalizeColor(chance.color())
  const seeded = normalizeColor(new Chance(userId).color())
  if (seeded !== '#ff00ff') return seeded
  const hash = crypto.createHash('md5').update(userId).digest('hex')
  return normalizeColor(hash.slice(0, 6))
}
...
function connection (socket) {
  ...
  parseNoteIdFromSocket(socket, function (err, noteId) {
    ...
    // random color
    let color = normalizeColor(chance.color())
    ...
    if (notes[noteId]) {
      ...
      do {
        ...
        if (found) {
          color = normalizeColor(chance.color())
          randomcount++
        }
